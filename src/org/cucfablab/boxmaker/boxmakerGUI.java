...
public class boxmakerGUI {
	...
	private Button btnLoadsessionfromxml;
	...
	private void setFields(userSettings us) {
		enterHeight.setText((new Double(us.getHeight())).toString());
		enterWidth.setText((new Double(us.getWidth())).toString());
		enterDepth.setText((new Double(us.getDepth())).toString());
		enterCutWidth.setText((new Double(us.getCutWidth())).toString());
		enterSlotWidth.setText((new Double(us.getSlotWidth())).toString());
		enterSlotWidth.setText((new Double(us.getSlotWidth())).toString());
		// / need to set the selection text for material....
		btnPortrait.setSelection(us.getPortrait());

		if (us.getUseInches()) {
			btnRadioButtonUnitsMM.setSelection(false);
			btnRadioButtonUnitsInches.setSelection(true);
		} else {
	...
	private class mySelectionAdapter extends SelectionAdapter {
		...
		public void widgetSelected(SelectionEvent e) {
			...
			if (e.getSource().getClass().getName().contains("Button")) {
				...
				if (b.getToolTipText().contains("Reset")) {
					...
					// create a separate splash screen and display the user
					// instructions
					splashScreen s1 = new splashScreen("Box Maker Parameters",
							"docs/BoxParms1.htm");
					s1.postIt();
				} else if (b.getToolTipText().contains("Help")) {
					// create a separate splash screen and display the general
					// informations
					splashScreen s1 = new splashScreen("About BoxMaker",
							"docs/Information_about_Boxmaker.htm");
					s1.postIt();
				} else if (b.getToolTipText().contains("SaveXML")) {
					// dialog to select file, then save
					Display display = Display.getDefault();
					Shell shell = new Shell(display);
					mySafeSaveDialog ssfile = new mySafeSaveDialog(shell);

					ssfile.open();

					String extra = ".xml";
					if (ssfile.getFileName().endsWith(".xml")) {
						extra = "";
					}

					us.saveToFile(ssfile.getFilterPath() + "/"
							+ ssfile.getFileName() + extra);

				} else if (b.getToolTipText().contains("LoadXML")) {
					// dialog to select file, then read

					Display display = Display.getDefault();
					Shell shell = new Shell(display);
					mySafeOpenDialog ssfile = new mySafeOpenDialog(shell);

					ssfile.open();

					if (ssfile.getFileName() != null) {
						System.out.println("xml file name is "
								+ ssfile.getFileName());
						us.loadSettings(ssfile.getFilterPath() + "/"
								+ ssfile.getFileName());
					...
					// if 'not isOK' then the testConstraints will have
					// displayed a dialog
					// to do: check behavior when !isOK
					if (isOK) {
						File bf = new File("BOX.jar");
						if (!bf.exists()) {
							// fail here because the required program is not
							// found
						...
						// retrieve the parameters from the userSEttings,
						...
						try {
							...
							// print the output of the process, if any, to the
							// console (mainly for debugging)
							String scan = "";
							...
							sc.close();
							if (lns > 0) {
								// if there is any output, there was an error,
								// so pop up a dialog
							...
							// the fork/exec raised an exception, job failed.
							// Pop up a dialog with the exception.
							Status status = new Status(IStatus.ERROR,
									"BoxMakerGUI", 0,
									"BoxMaker Program Exec Failed", null);
							ErrorDialog.openError(Display.getCurrent()
						...
						// Process completed, result should be in file called
						// 'boxmaker.pdf'
						...
						if (!failed) {
							...
							// pop up a 'safe save dialog', rename the default
							// file
							Display display = Display.getDefault();
							Shell shell = new Shell(display);
							...
							// note: remember the file name so we can find the
							// most recent result...
							us.setUserSavedFile(ssfile.getFilterPath() + "/"
									+ ssfile.getFileName() + extra);

							// enable the preview button now that there is
					...
					// Select the units. Numbers entered will be converted to
					// millimeters if needed
					if (btnRadioButtonUnitsMM.getSelection()) {
						us.setUseInches(false);
					} else {
						us.setUseInches(true);
					}
				} else if (b.getToolTipText().contains("ToggleLandscape")) {
					if (us.getPortrait()) {
						us.setPortrait(false);
						btnPortrait.setSelection(false);
					} else {
						us.setPortrait(true);
						btnPortrait.setSelection(true);
					}
				}
		...
		private class mySafeOpenDialog {
			// The wrapped FileDialog
			private FileDialog dlg;
			...
			public mySafeOpenDialog(Shell shell) {
				dlg = new FileDialog(shell, SWT.READ_ONLY);
			}
				...
				while (!done) {
					...
					if (fileName == null) {
						...
						if (!file.exists()) {
							...
							// We really should read this string from a
							// resource bundle
							mb.setMessage(fileName + " does not exist.");

							// If they click Yes, we're done and we drop out. If
							// they click No, we redisplay the File Dialog
							done = mb.open() == SWT.RETRY;
						} else {
							// File does exist, so drop out
							done = true;
						}
					}
				}
	...
	protected void createContents() {
		...
		btnPortrait.setToolTipText("ToggleLandscape");
		...
		btnSavesession = new Button(shlBoxmaker, SWT.NONE);
		btnSavesession.setToolTipText("SaveXML");
		btnSavesession.setFont(SWTResourceManager.getFont("Arial", 14,
				SWT.NORMAL));
		btnSavesession.setText("Save");
		btnSavesession.addSelectionListener(msa);
		new Label(shlBoxmaker, SWT.NONE);
		new Label(shlBoxmaker, SWT.NONE);
		new Label(shlBoxmaker, SWT.NONE);
		new Label(shlBoxmaker, SWT.NONE);
		...
		btnLoadsessionfromxml = new Button(shlBoxmaker, SWT.NONE);
		btnLoadsessionfromxml.setToolTipText("LoadXML");
		btnLoadsessionfromxml.setText("Load");
		btnLoadsessionfromxml.setFont(SWTResourceManager.getFont("Arial", 14,
				SWT.NORMAL));
		btnLoadsessionfromxml.addSelectionListener(msa);

		Button btnQuit = new Button(shlBoxmaker, SWT.NONE);
		btnQuit.setLayoutData(new GridData(SWT.CENTER, SWT.CENTER, false,
				false, 1, 1));
